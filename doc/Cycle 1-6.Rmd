...
There are a few industries that stand out which we will take a closer look at. We first notice that the CMM (Computer and Mathematical Occupations) data, which we are taking as "high-tech", does not seem to support the statement made in the USA Today article. We will also look at FIN (Financial Specialists), the only occupational category in which whites appear to make more than their Asian counterparts. Last, we will look at MED (Healthcare Practitioners and Technical Occupations) because it has the largest percentage difference in mean salary between Asians and whites. 
...
This is interesting. We see that these findings conflict with what we observed about the CMM/tech occupations. Although there is still a higher proportion of Master's degree holders amongst the Asians in FIN, they have, on average, a lower salary than their white counterparts. In fact, many white individuals in FIN only hold Bachelor's degrees.
...
This graph appears to be misleading due to Texas being an outlier. In the data, there is only one listed individual under "Asian" + "FIN", thus leading to an "average" salary of 300K+ to represent the entire demographic. With that one exception, however, we see that the overall finding (whites making more than their Asian counterparts in FIN) appears to be true across almost all individual states. Georgia (51275.41) and the District of Columbia (24938.96) are the only two significant exceptions.
...
The findings on these graphs are interesting. We know that MED had the largest overall difference in salaries between Asians and whites, yet there is a significantly larger proportion of Master's degree holders within the white MED demographic. This may be due to the fact that medical degrees themselves do not count towards a higher level of education. Those that hold Doctorate's or Master's degrees most likely received degrees that do not contribute directly to their positions as doctors, surgeons, etc. That may explain why the differences in degrees holders here do not correlate with our overall findings.
